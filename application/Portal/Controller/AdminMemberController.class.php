...
class AdminMemberController extends AdminbaseController {
	...
	function _initialize() {
		parent::_initialize();
		$this->member_model = D("Portal/Posts");
	...
	public function add_post(){
		if (IS_POST) {
			...
			$result=$this->member_model->add($article);
			if ($result) {
	...
	public function edit(){
		...
		$this->assign("post",$member);
		$this->assign("smeta",json_decode($member['smeta'],true));
		$this->assign("terms",$terms);
	...
	public function edit_post(){
		if (IS_POST) {
			$member_id=intval($_POST['post']['id']);
			$_POST['smeta']['thumb'] = sp_asset_relative_url($_POST['smeta']['thumb']);
			...
			$article['smeta']=json_encode($_POST['smeta']);
			$article['post_content']=htmlspecialchars_decode($article['post_content']);
			$result=$this->member_model->save($article);
			if ($result!==false) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	private function _lists($where=array()){
		$term_id=I('request.term',4,'intval');
		...
		if(!empty($term_id)){
		    $this->member_model->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id");
		}
		...
		if(empty($term_id)){
		    $this->member_model->field('a.*,c.user_login,c.user_nicename');
		}else{
		    $this->member_model->field('a.*,c.user_login,c.user_nicename,b.listorder,b.tid');
		    $this->member_model->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id");
		}
		$member=$this->member_model->where(array('post_type'=>3))->select();

		$this->assign("page", $page->show('Admin'));
		$this->assign("formget",array_merge($_GET,$_POST));
		$this->assign("posts",$member);
	}
	...
	public function delete(){
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			if ($this->member_model->where(array('id'=>$id))->save(array('post_status'=>3)) !==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		...
		if(isset($_POST['ids'])){
			...
			if ($this->member_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>3))!==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
	...
	public function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			if ( $this->member_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>1)) !== false ) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			if ( $this->member_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>0)) !== false) {
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
	...
	public function top(){
		if(isset($_POST['ids']) && $_GET["top"]){
			...
			if ( $this->member_model->where(array('id'=>array('in',$ids)))->save(array('istop'=>1))!==false) {
				$this->success("置顶成功！");
			} else {
				$this->error("置顶失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["untop"]){
			...
			if ( $this->member_model->where(array('id'=>array('in',$ids)))->save(array('istop'=>0))!==false) {
				$this->success("取消置顶成功！");
			} else {
				$this->error("取消置顶失败！");
			}
	...
	public function recommend(){
		if(isset($_POST['ids']) && $_GET["recommend"]){
			...
			if ( $this->member_model->where(array('id'=>array('in',$ids)))->save(array('recommended'=>1))!==false) {
				$this->success("推荐成功！");
			} else {
				$this->error("推荐失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["unrecommend"]){
			...
			if ( $this->member_model->where(array('id'=>array('in',$ids)))->save(array('recommended'=>0))!==false) {
				$this->success("取消推荐成功！");
			} else {
				$this->error("取消推荐失败！");
			}
			...
			if(isset($_GET['ids']) && $_GET['old_term_id'] && isset($_POST['term_id'])){
			    ...
			    if($old_term_id!=$term_id){
			        ...
			        foreach ($ids as $id){
                      ...
                      $this->member_model->save(array('id'=>$id,'post_type'=>1));
			            }
			        }
	...
	public function copy(){
	    if(IS_POST){
	        if(isset($_GET['ids']) && isset($_POST['term_id'])){
	            ...
	            $term_id=I('post.term_id',4,'intval');
	            ...
	            foreach ($ids as $id){
	                $find_member=$this->member_model->field('post_keywords,post_content,post_title,jingyan,leixing,post_excerpt,smeta')->where(array('id'=>$id))->find();
	                if($find_member){
	                    $find_member['post_author']=$uid;
                      $find_member['post_type']=3;
	                    $find_member['post_date']=date('Y-m-d H:i:s');
	                    $find_member['post_modified']=date('Y-m-d H:i:s');
	                    $member_id=$this->member_model->add($find_member);
	                    if($member_id>0){
	                      array_push($data, array('object_id'=>$member_id,'term_id'=>$term_id));
	                    }
	                }
	            }
	        ...
	        $tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
	        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
	        $terms = $this->terms_model->where(array("taxonomy"=>"works"))->where(array('term_id'=>4))->order(array("path"=>"ASC"))->select();
	        $new_terms=array();
	        foreach ($terms as $r) {
	            $r['id']=$r['term_id'];
	            $r['parentid']=$r['parent'];
	...
	public function clean(){
		if(isset($_POST['ids'])){
			$ids = I('post.ids/a');
			$ids = array_map('intval', $ids);
			$status=$this->member_model->where(array("id"=>array('in',$ids),'post_status'=>3))->delete();
			...
			if(isset($_GET['id'])){
				$id = I("get.id",0,'intval');
				$status=$this->member_model->where(array("id"=>$id,'post_status'=>3))->delete();
	...
	public function restore(){
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			if ($this->member_model->where(array("id"=>$id,'post_status'=>3))->save(array("post_status"=>"1"))) {
				$this->success("还原成功！");
			} else {
				$this->error("还原失败！");
			}
